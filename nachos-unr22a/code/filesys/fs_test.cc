...
Copy(const char *from, const char *to)
{
    ...
    fileSystem->Close(openFile->GetSector());
    delete openFile;
    fclose(fp);
}
...
{
    fileSystem->Print();
    ASSERT(fileSystem->CreateDir("trin"));
    ASSERT(fileSystem->CD("trin"));
    ASSERT(fileSystem->CreateDir("petru"));
    ASSERT(fileSystem->Create("../hola.txt"));
    ASSERT(fileSystem->Create("hola2.txt"));
    ASSERT(fileSystem->CD(".."));
    ASSERT(fileSystem->Create("trin/petru/holaPetru.txt"));
    ASSERT(fileSystem->CD("trin"));
    ASSERT(fileSystem->CD("/"));
    ASSERT(!fileSystem->RemoveDir("trin/petru"));
    OpenFile * open = fileSystem->Open("trin/petru/holaPetru.txt");
    ASSERT(open != nullptr);
    ASSERT(fileSystem->Close(open->GetSector()));
    delete open;
    ASSERT(fileSystem->Remove("trin/petru/holaPetru.txt"));
    ASSERT(fileSystem->RemoveDir("trin/petru"));
    fileSystem->Print();
